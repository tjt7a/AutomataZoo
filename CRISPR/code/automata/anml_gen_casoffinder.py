...
if len(sys.argv) != 3 :
    print "USAGE: python anml_gen_casoffinder.py <infile> <dist1>"
    sys.exit(1)

filename = sys.argv[1]
dist1 = int(sys.argv[2])

def start(start_id,length, dist, character_upper, character_lower):
    if dist>=2 and dist <= length/2:
        for i in range(1,length+1):
            if i%length == 1:
                ...
                print '                <activate-on-match element="%d"/>' %(start_id+900)  
...
with open(filename) as f:
    ...
    for line in f:
        ...
        start(i*900,12,dist1,character_upper[8:20],character_lower[8:20])
        pam(900*(i+1),character_upper[20:], character_lower[20:])
        i=i+1
